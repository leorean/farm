		...
		if (_name == "OBJ") {
			...
			for (var _i = _seg_x; _i < _seg_x + (V_WIDTH div _t); _i++) {
				for (var _j = _seg_y; _j < _seg_y + (V_HEIGHT div _t); _j++) {
					...
					if (_type == "rock_big") {
						create_harvestable(_x + 16, _y + 16,  HType.ROCK_BIG, _tile);
					}
					if (_type == "wood_big") {
						create_harvestable(_x + 16, _y + 16,  HType.WOOD_BIG, _tile);
					}
					if (_type == "bush_big") {
						create_harvestable(_x + 16, _y + 16,  HType.BUSH_BIG, _tile);
					}
